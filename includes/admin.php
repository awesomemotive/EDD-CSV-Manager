 ...
 * @return		void
 */
function edd_csv_exporter_metabox() { 
	?>
	<div class="postbox">
		<h3><span><?php _e( 'Export Products to CSV', 'edd-csv-importer' ); ?></span></h3>
		<div class="inside">
			<p><?php _e( 'Export products from your Easy Digital Downloads site to a .csv file.', 'edd-csv-importer' ); ?></p>
			<form method="post" enctype="multipart/form-data" action="<?php echo admin_url( 'tools.php?page=edd-settings-export-import' ); ?>">
				<?php
					echo '<p><input type="hidden" name="edd_action" value="export_csv" />';
					submit_button( __( 'Export', 'edd' ), 'secondary', 'submit', false );
					echo '</p>';
				?>
			</form>
		</div>
...
add_action( 'edd_export_import_top', 'edd_csv_exporter_metabox' );
 ...
 * @return		void
 */
function edd_export_csv() {
	// Make sure we're allowed to export...
	if( !current_user_can( 'manage_options' ) )
		wp_die( __( 'You do not have permission to export data.', 'edd-csv-importer' ), __( 'Error', 'edd-csv-importer' ) );
...
add_action( 'edd_export_csv', 'edd_export_csv' );
	...
	foreach ( $csv->data as $key => $row ) {
		...
		if( $post_id ) {
			...
			if( $image ) {
				...
				$attachment_id = wp_insert_attachment( $attachment, $final_images[0]['url'], $post_id );
